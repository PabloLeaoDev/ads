...
class Mob(pygame.sprite.Sprite):
    def __init__(self, mob_type):
        ...
        if mob_type == 'normal':
            self.image = pygame.image.load(resource_path('assets/mob_normal.png')).convert_alpha()
        elif mob_type == 'fast':
            self.image = pygame.image.load(resource_path('assets/mob_fast.png')).convert_alpha()
            self.width = 45
            self.height = 45
        elif mob_type == 'tank':
            self.width = 80
            self.height = 80
            self.image = pygame.image.load(resource_path('assets/mob_tank.png')).convert_alpha()
